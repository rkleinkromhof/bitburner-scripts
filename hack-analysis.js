...
function mainLoop() {
	...
	const analyses = Array.prototype.slice.call(availableServers)
		.filter(server => server.requiredHackingSkill <= hackSkill)
	...
	for (let i = 0; i < analyses.length; i++) {
		...
		// log(`(${String.prototype.padStart.call(i+1, 2)}) ${server.hostname} - (${server.requiredHackingSkill}) runtime ${formatDuration(analysis.totalRuntime)}, est. profitability = ${formatMoney(analysis.profitPerSec)}/s (${formatMoney(analysis.hackMoney)}) or ${formatMoney(analysis.profitPerMb)}/MB (${formatRam(analysis.totalRamCost)}), efficiency = ${analysis.profitabilityEfficiency}/s/MB`);
		// log(`(${String.prototype.padStart.call(i+1, 2)}) ${server.hostname} - (${server.requiredHackingSkill}) (${formatPercent(analysis.hackMoneyPercentage)}) runtime ${formatDuration(analysis.totalRuntime)}, est. profitability = ${formatMoney(analysis.profitPerSec)}/s (${formatMoney(analysis.hackMoney)}) or ${formatMoney(analysis.profitPerMb)}/MB (${formatRam(analysis.totalRamCost)}), efficiency = ${analysis.profitabilityEfficiency}/s/MB`);
		// log(`(${String.prototype.padStart.call(i+1, 2)}) ${server.hostname} - (${server.requiredHackingSkill}) (${formatPercent(analysis.hackMoneyPercentage)}) runtime ${formatDuration(analysis.totalRuntime)}, est. profitability = ${analysis.profitPerSec}/s (${analysis.hackMoney}) or ${analysis.profitPerMb}/MB (${analysis.totalRamCost}), efficiency = ${analysis.profitabilityEfficiency}/s/MB`);
		if (server.moneyMax > 0) {
			analysis.printProps(log);
		}
	}
}
