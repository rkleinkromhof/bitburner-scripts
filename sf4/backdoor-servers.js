 ...
 * Backdoor the thing.
 * @param {NS} ns Namespace
 * @param {String[]} path The path of servers to take to the target server
 **/
async function backdoorTarget(ns, path) {
	let connectionOk = true;
	let node;

	while(connectionOk && (node = path.shift())) {
		connectionOk = ns.connect(node);
	}

	if (!path.length && connectionOk) {
		ns.tprint(`Installing backdoor at ${node}`);
